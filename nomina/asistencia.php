                        ...
                        <td>
                        	<table cellpadding="0" cellspacing="0">
                            	<tr>
                                	<td>
                                        De<br/>
										<input type="text" id="fecIni" style="margin-bottom:0px; width:80%"  autocomplete="off" /><i class="btn-icon-only icon-calendar" style="font-size:16px"> </i>
                                    </td>
                                    <td>
                                        Al<br/>
										<input type="text" id="fecFin" style="margin-bottom:0px; width:80%" autocomplete="off" /><i class="btn-icon-only icon-calendar" style="font-size:16px"> </i>
                                    </td>
                                </tr>
                            </table>
                        </td>
...
<script type="text/javascript">	
		...
		switch (opc) {
				case 'asistencia':
					...
					$("#displaySetIngresoXFalta").hide();
								...
								$("#tituloControl").html("Registro de Asistencia");
									$.each(empleado.registroAsistencia, function (i, item) { 
						 templateItem += `
						 		<tr>
								 		<td>${item.fecha}</td>
										 <td>${item.hora}</td>
			...
			case 'retardos':
				$("#tituloControl").html('Registro de Rertardos');	
					...
					$.each(empleado.diasRetardo, function (i, item) { 
						...
						$("#displaySetIngresoXFalta").hide();
						 templateItem += `
						 		<tr>
								 		<td>${item.fecha}</td>
										 <td>${item.hora}</td>
										 <td> <input type="number" id="sancion_${empleadoId}_${item.aplicaIncidencia}" value="${item.monto}"> </td>
										 <td><button onclick="removeIncidenciaAsistencia( {
												 tipo: 'retardo',
												 monto: ${item.monto},
												 empleado: ${empleadoId},
												 incidencia: ${item.aplicaIncidencia},
												 checado: '${item.fecha} ${item.hora}',
												 index: ${i}
			...
			case 'faltas':
					...
					$.each(empleado.diasFaltas, function (i, item) { 
						 templateItem += `
						 		<tr>
								 		<td>${item.fecha}</td>
										 <td> - </td>
	...
	function renderListaAsistencia( lista ){
		...
		for(i in lista){
				...
				echo+= `	<td style='text-align:center;cursor:pointer' onclick='controlAsistencia("asistencia",${lista[i].id})'>${lista[i].historialAsistencia.length}</td>`;
				echo+= `	<td style='text-align:center;cursor:pointer' onclick='controlAsistencia("retardos",${lista[i].id})'>${lista[i].nRetardos}</td>`;
				echo+= `	<td style='text-align:center;cursor:pointer' onclick='controlAsistencia("faltas",${lista[i].id})'>${lista[i].faltas}</td>`;
