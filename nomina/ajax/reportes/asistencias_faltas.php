...
$diaActual = date('d');
if ( $diaActual <= 15) {
    $reporte->preparaReporte($reporte->generaFaltasRetardos(), array('fechaInicio' => date('Y-m-1'),'fechaFin' => date('Y-m-15') ) ); 
}else{
    $diasMes = cal_days_in_month(CAL_GREGORIAN,date('m',date('Y')));
    $reporte->preparaReporte($reporte->generaFaltasRetardos(), array('fechaInicio' => date('Y-m-16'),'fechaFin' => date('Y-m-'.$diasMes) ) ); 
}
   $configCorreo = array("descripcionDestinatario" => "Nomina Matrix",
                                        "mensaje" => "...",
                                        "pathFile" => "Reporte retardos y faltas.xlsx",
                                        "subject" => "Faltas y Retardos Matrix -Leon",
                                        "correos" => array( "auxsistemas@matrix.com.mx","rh@matrix.com.mx","sestrada@matrix.com")
