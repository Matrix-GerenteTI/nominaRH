...
function generarPDF($serie,$folio){
	...
	$pdf->Row(array('','', 'RECIBO DE NOMINA', $row['serie'].'-'.$row['folio']));
	$pdf->Ln(2);
	...
	$pdf->Cell(95,5,'Pag. 1/1',0,0,'R');
	
	$pdf->SetXY(145,-45);
	$pdf->SetFont('Arial','b',8);
	$pdf->MultiCell(62,3,utf8_decode('RECIBÍ DE '.strtoupper($EmisorNombre).' EL TOTAL NETO SEÑALADO Y ESTOY DE ACUERDO CON LO INDICADO.'),0,'C');
	...
	$pdf->Output("CFDIS/".$EmisorRFC."/".$serie."".$folio.".pdf",'F');
}
?>