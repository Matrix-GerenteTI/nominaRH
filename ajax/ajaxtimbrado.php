<?php
	...
	switch($op){
		...
		case "cmbNomina":{
			...
			$query = "SELECT DISTINCT fechainicial,fechafinal
					  FROM 		pnomina
					  WHERE 	status=1 
					  ";
			$sql = $conexion->query($query);
			while($row = $sql->fetch_assoc()){
				$echo.= '<option value="'.formateaFechaGUION($row['fechainicial']).'_'.formateaFechaGUION($row['fechafinal']).'">'.formateaFechaGUION($row['fechainicial']).' - '.formateaFechaGUION($row['fechafinal']).'</option>';
			}
			echo $echo;
			break;
		}
		...
		case "dias":{
			echo calculaDias($_POST['fechaIni'],$_POST['fechaFin']);
			break;
		}
		...
		case "buslistaTimbrado":{
			...
			$query = "SELECT 	e.nip as nip,
					  ...
					  AND		e.status=1 
					  ORDER BY d.descripcion,e.nombre";
			$sql = $conexion->query($query);
			while($row = $sql->fetch_assoc()){
				$array[$n] = $row;
