<?php
	...
	switch($op){
		...
		case "timbrar":{
			...
			if($r1['idsucursal']==8 || $r1['idsucursal']==9){
				$EmisorRegimen = 'A6215457107';
			}else{
				$EmisorRegimen = 'A6841538106';
			}
			...
			*/
			...
			$invoice_path = "./send/".$EmisorRFC."/".$serie."".$folio.".zip";
			...
			//$password = 'P4ssw0rd+';	
			$url = 'https://sat.sifei.com.mx:8443/wsSIFEI/SIFEI_Service?wsdl'; //'http://devcfdi.sifei.com.mx:8080/SIFEI/SIFEI?wsdl';
			$username = 'GODR770917KK6';
			$password = 'Kwc6021709@';
			$serie2 = '';
			$idEquipo = 'YmU5ZGNmZmEtNjczNC0yNzU3LTM0MTUtNTBlNGJkYzNlODQz'; //'OWM3N2U3MmQtZGU0ZC1jNjQ4LTE2MjAtNGZkY2FhNWE2ZTNk';
			...
			$client = new SoapClient('https://sat.sifei.com.mx:8443/wsSIFEI/SIFEI_Service?wsdl', array('trace' => 1));
			...
			$response = $client->__soapCall("getCFDI", array($params));
			print_r($response);
			$xmlResp = $response->return;
			
			$fch= fopen("CFDIS/".$EmisorRFC."/".$serie.$folio.'.zip', "w"); // Abres el archivo para escribir en él
			fwrite($fch, $xmlResp); // Grabas
			...
			*/
			...
			$pos2 = strpos($resultado, "timbrado</string>");
			
			//print_r($response);
			//$xmlResp = $response->stampResult->xml;
			if($pos2 !== false){
				
				$sxe = simplexml_load_file("CFDIS/".$EmisorRFC."/".$serie."".$folio.".xml");
