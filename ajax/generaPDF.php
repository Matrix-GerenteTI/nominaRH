...
class PDF extends FPDF{
	...
	foreach($arr as $val){
		...
		$pdf->Row(array('','', 'RECIBO DE NOMINA', 'Serie: '.$row['serie'].' | Folio:'.$row['folio']));
		$pdf->Ln(2);
		...
		$pdf->Row(array('','', $EmisorRFC, $row['noCertificado']));
		$pdf->SetFonts(array(array('Arial','',8),array('Arial','',8),array('Arial','',8),array('Arial','b',8)));
		$pdf->Row(array('','', strtoupper($rEmisor['calle'].' '.$rEmisor['numext'].', '.$rEmisor['colonia'].' C.P. '.$rEmisor['cp'].'.'), 'Fecha y Hora de Emision'));
		$pdf->SetFonts(array(array('Arial','',8),array('Arial','',8),array('Arial','',8),array('Arial','',8)));
		$pdf->Row(array('','', strtoupper($rEmisor['municipio'].', '.$rEmisor['idestado']), $row['fecha'].'T'.$row['hora']));
		$pdf->SetFonts(array(array('Arial','',8),array('Arial','',8),array('Arial','',8),array('Arial','b',8)));
		...
		$pdf->Ln(4);
		$pdf->SetFont('Arial','b',8);
		$pdf->Cell(20,5,"Tipo Nomina:",0,0,'L');
		$pdf->SetFont('Arial','',8);
		if($NominaTipoNomina=='O')
			$pdf->Cell(50,5,'ORDINARIA',0,0,'L');
		else	
			$pdf->Cell(50,5,'EXTRAORDINARIA',0,0,'L');
		$pdf->SetFont('Arial','b',8);
		$pdf->Cell(25,5,"Dias Pagados:",0,0,'L');
		$pdf->SetFont('Arial','',8);
		$pdf->Cell(37,5,$NominaDiasPagados,0,0,'L');		
		$pdf->SetFont('Arial','b',8);
		$pdf->Cell(25,5,"Fecha de Pago",0,0,'L');
		$pdf->SetFont('Arial','',8);
		$pdf->Cell(37,5,$pdf->formateaFecha($NominaFechaPago),0,0,'L');
		$pdf->Ln(4);
		$pdf->SetFont('Arial','b',8);
		$pdf->Cell(28,5,"Fecha Inicio Pago:",0,0,'L');
		$pdf->SetFont('Arial','',8);
		$pdf->Cell(42,5,$pdf->formateaFecha($NominaFechaInicialPago),0,0,'L');
		$pdf->SetFont('Arial','b',8);
		$pdf->Cell(25,5,"Fecha Final Pago:",0,0,'L');
		$pdf->SetFont('Arial','',8);
		$pdf->Cell(37,5,$pdf->formateaFecha($NominaFechaFinalPago),0,0,'L');		
		$pdf->SetFont('Arial','b',8);
		...
		if(count($arrOtrosPagos)>0){
			...
			$pdf->Cell(20,5,"",'BT',0,'C');
			$pdf->Cell(134,5,"",'BT',0,'C');	
			$pdf->Cell(20,5,number_format($NominaTotalOtrosPagos,2,'.',','),'RBT',1,'R');
		}
		$pdf->Ln(3);
		$TotalIncidencias = 0;
		...
		$pdf->Ln(3);
		$pdf->SetFont('Arial','',7);
		$pdf->Cell(57,5,$row['wsuuid'],0,0,'L');
		$pdf->Cell(50,5,$row['wsnoCertificado'],0,0,'L');
		$pdf->Cell(50,5,$row['wsfecha'],0,0,'L');
		$pdf->Cell(50,5,$row['wsrfc'],0,0,'L');
		$pdf->Ln(5);
		...
		$pdf->SetFont('Arial','',6);
		$pdf->Cell(29,2,"",0,0,'L');
		$pdf->MultiCell(107,2,$row['wssellocfd'],0,'L');
		$pdf->SetFont('Arial','b',6);
		$pdf->Cell(29,3,"",0,0,'L');
		...
		$pdf->SetFont('Arial','',6);
		$pdf->Cell(29,2,"",0,0,'L');
		$pdf->MultiCell(107,2,$row['wssellosat'],0,'L');
		$pdf->SetFont('Arial','b',6);
		$pdf->Cell(29,3,"",0,0,'L');
		$pdf->Cell(107,3,"Cadena Original:",0,0,'L');
		$pdf->Ln(3);
		$pdf->SetFont('Arial','',6);
		$pdf->MultiCell(136,2,utf8_decode($row['cadenaOriginal']),0,'L');		
		$pdf->Ln(2);
		$pdf->SetFont('Arial','b',8);
		...
		$pdf->SetXY(145,-45);
		$pdf->SetFont('Arial','b',8);
		$pdf->MultiCell(62,3,'RECIBÍ DE '.strtoupper($EmisorNombre).' EL TOTAL NETO SEÑALADO Y ESTOY DE ACUERDO CON LO INDICADO.',0,'C');
