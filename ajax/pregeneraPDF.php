...
class PDF extends FPDF{
	...
	function CalculaAntiguedadSAT( $fecha, $fechaFinal ) {
		...
		$fecha2 = new DateTime(date("Y-m-d",strtotime($fechaFinal."+ 1 days")));
	...
	foreach($arr as $idcontrato){
		...
		$NominaTipoNomina = $_GET['idtiponomina']=='A'?'ORDINARIA':'EXTRAORDINARIA';
		$NominaFechaInicialPago = $pdf->formateaFechaSLASH($_GET['fechaIni']);
		$NominaFechaFinalPago = $pdf->formateaFechaSLASH($_GET['fechaFin']);
		$NominaDiasPagados = $_GET['diasPagados'];
		$NominaFechaPago = $pdf->formateaFechaSLASH($_GET['fechaPago']);
		$NominaUUID = $_GET['uuidanterior'];
		...
		// 	   AND 		p.status=1";
		$q1 = "SELECT 	i.idincidencia as TipoPercepcion,
						i.idincidencia as Clave,
						tp.descripcion as Concepto,
						i.monto as ImporteGravado,
						'0' as ImporteExento,
						'0' as ValorMercado,
						'0' as PrecioAlOtorgarse
			   FROM 	pnomina n
			   INNER JOIN pnomina_incidencias i ON n.id=i.idpnomina
			   INNER JOIN ctipopercepcion tp ON i.idincidencia=tp.id
			   WHERE 	n.fechainicial='".$NominaFechaInicialPago."' 
			   AND 		n.fechafinal='".$NominaFechaFinalPago."' 
			   AND 		n.idempleado=".$nip."
			   AND 		i.tipoincidencia='percepcion'
			   AND 		n.status=1";
		$s1 = $conexion->query($q1);
		...
		// 	   AND 		fechaCargo>='".$NominaFechaInicialPago."' AND fechaCargo<='".$NominaFechaFinalPago."'
		// 	   AND 		d.status=1";
		//die($q3);
		$q3 = "SELECT 	i.idincidencia as TipoDeduccion,
						i.idincidencia as Clave,
						td.descripcion as Concepto,
						i.monto as Importe
			   FROM 	pnomina n
			   INNER JOIN pnomina_incidencias i ON n.id=i.idpnomina
			   INNER JOIN ctipodeduccion td ON i.idincidencia=td.id
			   WHERE 	n.fechainicial='".$NominaFechaInicialPago."'
			   AND 		n.fechafinal='".$NominaFechaFinalPago."' 
			   AND 		n.idempleado=".$nip."
			   AND 		i.tipoincidencia='deduccion'
			   AND 		n.status=1";
		//echo $q3;
		...
		//Header
		//INSERTAMOS EL LOGO
		$pdf->Image('logos/logo.jpg',10,10,40); //LOGO
		...
		//$pdf->Image("CBB.png",10,230,30);
		
		
		#DATOS DEL CFDI		
		$pdf->SetBorders(array(0,0,0,0,0));		
		$pdf->SetWidths(array(21,20, 104, 50));
		$pdf->SetAligns(array('C','C','L','R'));
		$pdf->SetFonts(array(array('Arial','',8),array('Arial','b',8),array('Arial','b',11),array('Arial','b',11)));
		$pdf->Row(array('','', 'RECIBO DE NOMINA', $serie.'-'.date('is')));
		$pdf->Ln(2);
		$pdf->SetAligns(array('C','L','L','R'));
		$pdf->SetHeights(3);
		$pdf->SetFonts(array(array('Arial','',8),array('Arial','',8),array('Arial','',8),array('Arial','b',8)));
		$pdf->Row(array('','', strtoupper($EmisorNombre), 'No. de Certificado SAT'));
		$pdf->SetFonts(array(array('Arial','',8),array('Arial','',8),array('Arial','',8),array('Arial','',8)));
		$pdf->Row(array('','', $EmisorRFC, $noCertificado));
		$pdf->SetFonts(array(array('Arial','',8),array('Arial','',8),array('Arial','',8),array('Arial','b',8)));
		$pdf->Row(array('','', strtoupper($rEmisor['calle'].' '.$rEmisor['numext'].', '.$rEmisor['colonia'].' C.P. '.$rEmisor['cp'].'.'), 'Fecha y Hora de Emision'));
		$pdf->SetFonts(array(array('Arial','',8),array('Arial','',8),array('Arial','',8),array('Arial','',8)));
		$pdf->Row(array('','', strtoupper($rEmisor['municipio'].', '.$rEmisor['idestado']), date("Y-m-d").'T'.date("H:i:s")));
		$pdf->SetFonts(array(array('Arial','',8),array('Arial','',8),array('Arial','',8),array('Arial','b',8)));
		$pdf->Row(array('','', 'Registro Patronal: '.$NominaEmisorRegistro, 'Lugar de Expedicion'));
		$pdf->SetFonts(array(array('Arial','',8),array('Arial','',8),array('Arial','',8),array('Arial','',8)));
		$pdf->Row(array('','', 'Regimen: '.$EmisorRegimen, $rEmisor['municipio']));
		...
		$pdf->SetFont('Arial','',8);
		$pdf->Cell(55,5,utf8_decode($ReceptorNombre),0,0,'L');
		$pdf->SetFont('Arial','b',8);
		...
		$pdf->SetFont('Arial','b',8);
		$pdf->Cell(25,5,utf8_decode("Antigüedad:"),0,0,'L');
		$pdf->SetFont('Arial','',8);
		...
		//echo $nip." - ".$NominaReceptorInicioLab."</br>";
		$pdf->Ln(4);
		$pdf->SetFont('Arial','b',8);
		...
		*/
		...
		$pdf->SetFont('Arial','',8);
		$pdf->Cell(55,5,utf8_decode($NominaReceptorPuesto),0,0,'L');
		$pdf->SetFont('Arial','b',8);
		$pdf->Cell(25,5,"Tipo Regimen:",0,0,'L');
		$pdf->SetFont('Arial','',8);
		$pdf->Cell(37,5,$NominaReceptorTipoRegimen,0,0,'L');		
		$pdf->SetFont('Arial','b',8);
		$pdf->Cell(25,5,"Tipo Nomina:",0,0,'L');
		$pdf->SetFont('Arial','',8);
		$pdf->Cell(45,5,$NominaTipoNomina,0,0,'L');
		/*
		$pdf->SetFont('Arial','b',8);
		...
		*/
		...
		$pdf->SetFont('Arial','',8);
		$pdf->Cell(45,5,$NominaReceptorTipoContrato,0,0,'L');		
		$pdf->SetFont('Arial','b',8);
		$pdf->Cell(25,5,"",0,0,'L');
		$pdf->SetFont('Arial','',8);
		$pdf->Cell(37,5,"",0,0,'L');		
		...
		$pdf->SetFont('Arial','b',8);
		$pdf->Cell(25,5,"Dias Pagados:",0,0,'L');
		
		$fecha_iniciolab = strtotime($pdf->formateaFecha($NominaReceptorInicioLab));
		$fecha_inicial = strtotime(str_replace("/","-",$_GET['fechaIni']));
		$fecha_final = strtotime($_GET['fechaFin']);
		if($fecha_iniciolab > $fecha_inicial){
			$fecInicioPago = $pdf->formateaFecha($NominaReceptorInicioLab);
			$diaspagados = intval(preg_replace('/[^0-9]+/', '', $pdf->CalculaAntiguedadSAT($NominaReceptorInicioLab,$NominaFechaFinalPago)), 10);
		}else{
			$fecInicioPago = $_GET['fechaIni'];
			$diaspagados = $_GET['diasPagados'];
		}
		
		$pdf->SetFont('Arial','',8);
		$pdf->Cell(37,5,$diaspagados,0,0,'L');		
		$pdf->Ln(4);
		$pdf->SetFont('Arial','b',8);
		$pdf->Cell(25,5,"Fecha de Pago",0,0,'L');
		$pdf->SetFont('Arial','',8);
		$pdf->Cell(45,5,$_GET['fechaPago'],0,0,'L');
		$pdf->SetFont('Arial','b',8);
		$pdf->Cell(25,5,"Fecha Inicio Pago:",0,0,'L');
		$pdf->SetFont('Arial','',8);
		$pdf->Cell(45,5,$fecInicioPago,0,0,'L');
		//1456898400
		/*$fecha_iniciolab = strtotime($pdf->formateaFecha($NominaReceptorInicioLab));
		$fecha_inicial = strtotime($_GET['fechaIni']);
			
		if($fecha_iniciolab > $fecha_inicial)
			$pdf->Cell(45,5,$pdf->formateaFecha($NominaReceptorInicioLab),0,0,'L');
		else
			$pdf->Cell(45,5,$_GET['fechaIni'],0,0,'L');
		*/
		$pdf->SetFont('Arial','b',8);
		$pdf->Cell(25,5,"Fecha Final Pago:",0,0,'L');
		$pdf->SetFont('Arial','',8);
		$pdf->Cell(37,5,$_GET['fechaFin'],0,0,'L');		
		/*
		$pdf->SetFont('Arial','b',8);
		$pdf->Cell(25,5,"Forma de Pago",0,0,'L');
		$pdf->SetFont('Arial','',8);
		$pdf->Cell(37,5,"EFECTIVO",0,0,'L');
		*/
		$pdf->Ln(6);
		...
		if(count($arrPercepcion)>0){
			...
			if(count($arrHorasExtra)>0){
				...
				foreach($arrHorasExtra as $row2){
					//$pdf->Row(array('','','Tipo de Horas: '.$row2['TipoHoras'].', D�as: '.$row2['Dias'].', Horas: '.$row2['HorasExtra'], '',''));
				}
			}
			$pdf->SetFont('Arial','b',8);
		...
		//Footer
		$pdf->SetY(-57);		
		$pdf->Line(10,221,205,221);
		$pdf->SetFont('Arial','b',7);
		$pdf->Cell(57,5,"Folio Fiscal",0,0,'L');
		$pdf->Cell(50,5,"Certificado SAT",0,0,'L');
		$pdf->Cell(50,5,"Fecha de Certificacion",0,0,'L');
		$pdf->Cell(50,5,"RFC del PAC",0,0,'L');
		$pdf->Ln(3);
		$pdf->SetFont('Arial','',7);
		$pdf->Cell(57,5,'',0,0,'L');
		$pdf->Cell(50,5,'',0,0,'L');
		...
		$pdf->Cell(29,5,"",0,0,'L');
		$pdf->Cell(107,5,"Sello del CFD:",0,0,'L');	
		$pdf->Ln(4);
		$pdf->SetFont('Arial','',6);
		$pdf->Cell(29,2,"",0,0,'L');
		$pdf->MultiCell(107,2,'',0,'L');
		$pdf->SetFont('Arial','b',6);
		$pdf->Cell(29,3,"",0,0,'L');
		$pdf->Cell(107,3,"Sello del SAT:",0,0,'L');	
		$pdf->Ln(3);
		$pdf->SetFont('Arial','',6);
		$pdf->Cell(29,2,"",0,0,'L');
		$pdf->MultiCell(107,2,'',0,'L');
		$pdf->SetFont('Arial','b',6);
		$pdf->Cell(29,3,"",0,0,'L');
		$pdf->Cell(107,3,"Cadena Original:",0,0,'L');
		$pdf->Ln(3);
		$pdf->SetFont('Arial','',6);
		$pdf->MultiCell(136,2,'',0,'L');		
		$pdf->Ln(2);
		$pdf->SetFont('Arial','b',8);
		$pdf->Cell(100,5,"Este documento es una representacion impresa de un CFDI",0,0,'L');
		$pdf->Cell(95,5,'Pag. '.$GLOBALS['paginas'].'/'.$paginas,0,0,'R');
		*/
		$pdf->SetXY(20,-45);
		$pdf->SetFont('Arial','b',6);
		$pdf->MultiCell(100,3,'RECIBI A MI MAS ENTERA SATISFACCION, NO QUEDANDOME A ADEUDAR CANTIDAD ALGUNA POR NINGUNA PRESTACION; ASI COMO NO SUFRI NINGUN ACCIDENTE O ENFERMEDAD PROFESIONAL A LA FECHA.',0,'L');
		
		$pdf->SetXY(145,-45);
		$pdf->SetFont('Arial','b',8);
		$pdf->MultiCell(62,3,'RECIBI DE CONFORMIDAD',0,'C');
		$pdf->Line(151,261,201,261);
		$pdf->SetXY(145,-17);
		$pdf->MultiCell(62,3,'FIRMA DEL EMPLEADO',0,'C');
