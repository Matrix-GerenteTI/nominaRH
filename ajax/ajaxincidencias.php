	...
	switch($op){
		case "lista":{
			...
			while($row2 =$sql2->fetch_assoc()){
				if($row2['Field']!='status' && $row2['Field']!='idcontrato' && $row2['Field']!='idaccion_correctiva' && $row2['Field']!='descripcion'){
			...
			while($row =$sql->fetch_assoc()){
				...
				$array[$it] = $row;				
				$array[$it]['permisos'] = $_SESSION['permisos'][8];
				$it++;
			}
		...
		case "add":{
			...
			$sql = $conexion->query($query);
			$conexion->query("INSERT INTO pbitacora (usuario,movimiento,modulo,query,fecha,hora,importancia) VALUES ('".$_SESSION['userid']."','AGREGA INCIDENCIA','INCIDENCIAS','".base64_encode($query)."',NOW(),NOW(),2)");
			if(!$sql){
				echo 0;
			}else{
				echo 1;
		...
		case "delete":{
			...
			$sql = $conexion->query($query);
			$conexion->query("INSERT INTO pbitacora (usuario,movimiento,modulo,query,fecha,hora,importancia) VALUES ('".$_SESSION['userid']."','ELIMINA INCIDENCIA','INCIDENCIAS','".base64_encode($query)."',NOW(),NOW(),3)");
			if(!$sql)
				echo 0;
			else
				echo 1;
		...
		case 'addDeduccion':{
			...
			$conexion->query("INSERT INTO pbitacora (usuario,movimiento,modulo,query,fecha,hora,importancia) VALUES ('".$_SESSION['userid']."','AGREGA INCIDENCIA','INCIDENCIAS','".base64_encode($quequeryInsertaDeduccionry)."',NOW(),NOW(),2)");
