...
switch ($opc) {
    ...
    break;
    case 'getImagenes':
        $directory = $_SERVER['DOCUMENT_ROOT']."/nomina/socioeconomico/".$_POST['empleado'];
        $imagenes = '';
        if (file_exists( $directory) ) {
            ...
            while (($archivo = $dirint->read()) != false)
            {
                if (strpos($archivo,'jpg') || strpos($archivo,'jpeg')){
                    ...
                    $imagenes.= '<a href="http://nominamatrix.ddns.net:8080/nomina/socioeconomico/'.$_POST['empleado'].'/'.$archivo.'" target="blank"><img src="http://nominamatrix.ddns.net:8080/nomina/socioeconomico/'.$_POST['empleado'].'/'.$archivo.'" style="width:200px; margin:10px" /></a>';
                }
            }
            $dirint->close();
            echo $imagenes;
